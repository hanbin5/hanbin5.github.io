...
| frontmatter                                | 결과                                          |
|--------------------------------------------|-----------------------------------------------|
| `publish: false` (또는 누락)               | Vault에서만 보임. 사이트에는 절대 안 올라감.   |
| `publish: true` (draft 줄 없음/비움/false) | 다음 sync 때 공개.                            |
| `publish: true` + `draft: true`            | sync가 스킵 — "거의 다 썼지만 아직" 안전망.    |

`publish: true` 한 줄이 공개의 single source of truth입니다. draft
줄은 명시적으로 `true`일 때만 발행을 막는 brake로 동작합니다 — 비어
있거나 없으면 발행 대상으로 간주됩니다 (Templater가 draft 필드를
빠뜨리는 흐름을 그대로 받기 위함).
...
dek: "부제 한 줄"              # 선택
category: "Essay"              # 선택, 기본 "Essay" — `type:`도 alias로 받음
readtime: "8분 분량"           # 선택
tags: [project-tag]            # 선택, 빈 배열이면 "(unclassified)" 컬럼
issue: 4                       # 선택, Vol.II No.XX 표기
number: "No. 012"              # 선택, 비우면 자동 넘버링
publish: true                  # sync용 — 이 한 줄이 공개 결정
# draft: true                  # 선택 — true일 때만 발행을 막음
---
```
...
- **category**: **닫힌 enum** — `Essay` / `Note` / `Log` / `Review`.
  ...
  새 값을 추가하려면 `src/content/categories.ts`의 `CATEGORIES`를 먼저 수정.
  `type:`이라는 키도 alias로 받습니다 — 둘 중 하나만 적으면 됩니다.
...
1. **이미지는 vault의 `_Attachments/`에 둔다.**
   - Obsidian Settings → Files & Links → "Default location for new
     attachments"을 `_Attachments/`로 잡으면, drag&drop 시 자동으로
     거기로 떨어집니다.
   - 폴더 단위로 비공개를 분리하지 않습니다. published 노트에서 참조한
     이미지만 사이트에 올라가므로, 안 올리고 싶으면 `publish: true` 노트
     본문에서 그 이미지를 참조하지 마세요. (필요하면 ATTACHMENTS_VAULT
     환경변수로 다른 폴더를 가리키게 할 수도 있습니다 — 예:
     `ATTACHMENTS_VAULT=$VAULT/06_Public npm run sync`.)

2. **published된 노트가 본문에서 참조하는 이미지만** sync 시 git에 들어
   갑니다.
   - `![[slam-trajectory.png]]` ← `_Attachments/`에서 찾아 복사
   - `![[diagrams/architecture.png|시스템 구조도]]` ← 서브폴더 OK, alt 보존
   - 위 두 케이스 모두 sync 후 노트는 `![alt](/attachments/file.png)` 형태로
     자동 변환되어 Astro가 렌더링.
   - `_Attachments/`에 있어도 published 노트가 참조 안 하면 git에
     안 들어감 — repo는 "실제로 사이트에 보일 것"만큼만 큼.

3. **참조했는데 `_Attachments/`에 없는 파일은 경고만 찍고 넘어감.**
   - sync 출력에 `⚠️ missing-attachment: foo.png` 표시
   - 빌드는 성공하지만 사이트에선 깨진 이미지 자리에 alt 텍스트가 보이는
     "loud failure" 상태가 됨 → 글 발행 전에 발견하기 쉬움
   - 해결: 그 파일을 `_Attachments/`에 넣거나, 참조를 수정.
...
해결 방법: 대상 노트에 `publish: true`를 추가해 다음 sync에서 같이
발행되도록 하거나(draft 줄은 없어도 됨), 링크 자체를 표준 마크다운
(외부 URL)이나 평문으로 바꿉니다.
